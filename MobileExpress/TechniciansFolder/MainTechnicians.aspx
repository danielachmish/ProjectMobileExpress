...
<asp:Content ID="Content4" ContentPlaceHolderID="ContentPlaceHolder3" runat="server">
    ...
    <script>
        ...
        function updateLocationTracking(checkbox) {
            const isEnabled = checkbox.checked;

            // שליחת הבקשה לשרת
            $.ajax({
                url: 'Profile.aspx/UpdateLocationTracking',
                type: 'POST',
                contentType: 'application/json',
                data: JSON.stringify({ showLocation: isEnabled }),
                success: function (response) {
                    if (response.d.success) {
                        // הצגת הודעת הצלחה
                        showSuccessMessage("העדפות המיקום עודכנו בהצלחה");
                    } else {
                        // הצגת הודעת שגיאה
                        showErrorMessage(response.d.message || "אירעה שגיאה בעדכון העדפות המיקום");
                        // החזרת המתג למצב הקודם
                        checkbox.checked = !isEnabled;
                    }
                },
                error: function () {
                    showErrorMessage("אירעה שגיאה בתקשורת עם השרת");
                    checkbox.checked = !isEnabled;
                }
            });
        }
        ...
        function showSuccessMessage(message) {
            // הוסף את הקוד להצגת הודעת הצלחה בהתאם לעיצוב שלך
            alert(message);
        }
