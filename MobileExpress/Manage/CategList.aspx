<%@ Page Title="" Language="C#" MasterPageFile="~/Manage/MainMaster.Master" AutoEventWireup="true" CodeBehind="CategList.aspx.cs" Inherits="MobileExpress.Manage.CategList" %>
                            ...
                            <div class="columns-button-container">
                                ...
                                <div class="dropdown-menu p-3" aria-labelledby="dropdownMenuButton" dir="rtl">
                                    <div class="form-check">
                                        ...
                                        <label class="form-check-label ml-3">CatId</label>
                                    </div>
                                    <div class="form-check">
                                        <label class="switch">
                                        ...
                                        <label class="form-check-label ml-3">CatName</label>
                                    </div>
                                    <div class="form-check">
                                        <label class="switch">
            ...
            <form id="form1" runat="server">
                    ...
                    <table class="display table table-borderless ul-contact-list-table" id="contact_list_table" style="width: 100%; direction: rtl;">
                        ...
                        <tbody id="categoriesList">
                            <asp:Repeater ID="Repeater2" runat="server">
                                <ItemTemplate>
                                    <tr class="row-status" data-status='<%# Eval("Status") %>' data-categorie-id='<%# Eval("CatId") %>'>
                                        <td>
                                            <input type="checkbox" class="selectRow" value='<%# Eval("CatId") %>'></td>
                                        <td><%# Eval("CatId") %></td>
                                        <td><%# Eval("CatName") %></td>
                                        <td><%# Eval("DateAdd") %></td>
                                        ...
                                        <td class="action-buttons">
                                            ...
                                            <asp:LinkButton class="delete-button" ID="LinkButton1" runat="server" CommandArgument='<%# Eval("CatId") %>' OnClick="btnDelete_Click" OnClientClick="return confirm('Are you sure you want to delete this item?');">
                ...
                <div id="CategoriesModal" class="modal">
                    <div class="modal-content">
                        ...
                        <h2 id="modalTitle">הוספת eydurhv</h2>
                        <asp:HiddenField ID="hfCatId" runat="server" />
                        <div class="form-group">
                            <asp:Label AssociatedControlID="txtCatName" runat="server">שם מלא:</asp:Label>
                            <asp:TextBox ID="txtCatName" runat="server" required="required" CssClass="form-control form-control-rounded"></asp:TextBox>
                        </div>
                        <asp:Button ID="btnSave" runat="server" OnClick=" SaveCategories" Text="שמירה" CssClass="btn btn-primary" />
                    </div>
                </div>
...
<asp:Content ID="Content4" ContentPlaceHolderID="UnderFooter" runat="server">
    ...
    <script>
        ...
        // פונקציה לפתיחת מודאל הוספת קטגוריה
        function openModalAdd() {
            var modal = document.getElementById('CategoriesModal');
            var title = document.getElementById('modalTitle');
            var btnSave = document.getElementById('<%= btnSave.ClientID %>');
            title.innerText = 'הוספת קטגוריה';
            btnSave.value = 'הוסף קטגוריה';
            // ניקוי שדות הטופס
            document.getElementById('<%= hfCatId.ClientID %>').value = '';
            document.getElementById('<%= txtCatName.ClientID %>').value = '';
            modal.style.display = 'block'
        }

        function openModalEdit(categorie) {
            var modal = document.getElementById('CategoriesModal');
            var title = document.getElementById('modalTitle');
            var btnSave = document.getElementById('<%= btnSave.ClientID %>');
            ...
            document.getElementById('<%= hfCatId.ClientID %>').value = categorie.Id;
            document.getElementById('<%= txtCatName.ClientID %>').value = categorie.CatName;
            modal.style.display = 'block';
        }
        ...
        function saveCategories() {
            console.log("פונקציית savecategorie התחילה");
            var data = {
                CatId: $('#<%= hfCatId.ClientID %>').val(),
                CatName: $('#<%= txtCatName.ClientID %>').val(),
              
            };

            console.log("נתונים שנאספו:", JSON.stringify(data));

            var method = data.CatId === "" ? "POST" : "PUT";
            var url = method === "POST" ? "/api/Categories" : "/api/Categories/" + data.CatId;

            console.log(`שולח בקשת ${method} ל-${url}`);

            $.ajax({
                type: "POST",
                url: "/api/CategoriesController/Post",
                data: JSON.stringify({ categoriesData: data }),
        ...
        function refreshcategorieTable() {
            $.ajax({
                type: "GET",
                url: "/api/Categories",
                contentType: "application/json; charset=utf-8",
                ...
                success: function (categories) {
                    ...
                    for (var i = 0; i < categories.length; i++) {
                        ...
                        var row = "<tr>" +
                            ...
                            "<td>" + (categories.Status ? "פעיל" : "לא פעיל") + "</td>" +
                           
                            "<td><button onclick='editcategorie(" + categories.CatId + ")'>עריכה</button>" +
                            "<button onclick='deletecategorie(" + categories.CatId + ")'>מחיקה</button></td>" +
                            "</tr>";
        ...
        function editcategories(categorieId) {
            console.log('נכנס לפונקציית editcategorie');
            console.log('מזהה לקוח:', CatId);

            $.ajax({
                type: "GET",
                url: "/api/CategoriesController/Put" + CatId,
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (categorie) {
                    console.log('נתוני הלקוח התקבלו בהצלחה:', categorie);

                    $('#<%= hfCatId.ClientID %>').val(categorie.CatId);
                    console.log('CatId הוגדר:', categorie.CatId);

                    $('#<%= txtCatName.ClientID %>').val(categorie.CatName);
                    console.log('CatName הוגדר:', categorie.CatName);
            ...
            console.log('סיום פונקציית editcategorie');
        }
        ...
        function deletecategories(CatId) {
            if (confirm('Are you sure you want to delete this categories?')) {
                // קריאה לשרת למחיקת הקטגוריה (AJAX או POSTBACK)
                console.log('Deleting categories with ID:', CatId);
            }
        }
        ...
        function deleteSelectedRows() {
            ...
            if (selectedIds.length > 0) {
                if (confirm('Are you sure you want to delete the selected items?')) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "CategList.aspx/Deletecategories",
        ...
        // זהו קוד שמנהל את הייצוג החזותי של מעמד הקטגוריה (פעיל/לא פעיל)  AJAX כאשר מתבצעת החלפת המעמד.
        document.addEventListener('DOMContentLoaded', function () {
            ...
            rows.forEach(row => {
                ...
                button.addEventListener('click', function () {
                    ...
                    updateStatusInDatabase(categoriesId, newStatus);
                });
            });

            function updateStatusInDatabase(CatId, Status) {
                console.log("Updating status for CatId: " + CatId + " to Status: " + Status); // הדפסה לצורכי דיבוג

                $.ajax({
                    type: "POST",
                    url: "CategList.aspx/UpdatecategoriesStatus",
                    data: JSON.stringify({ CatId: CatId, Status: Status }),
                    contentType: "application/json; charset=utf-8",
                    dataType: "json",
                    success: function (response) {
